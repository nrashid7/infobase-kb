/**
 * Pilot Run #2 - Idempotency Test
 * 
 * Re-runs the exact same pilot crawl WITHOUT deleting the KB.
 * Tests deterministic claim_id stability.
 */
...
async function runPilot2() {
  ...
  console.log('  🧪 PILOT RUN #2 - IDEMPOTENCY TEST');
  console.log('═'.repeat(70));
  console.log(`\n  Domain: epassport.gov.bd`);
  console.log(`  maxDepth: 3`);
  console.log(`  maxPages: 30`);
  console.log(`  Method: Firecrawl MCP (agent-orchestrated)`);
  console.log(`  Pages to process: ${SCRAPED_PAGES.length}`);
  console.log(`  Note: KB NOT deleted between runs\n`);

  // Load KB (with existing data from Run #1)
  const kb = loadOrCreateKB(PATHS.kbPath, PATHS.kbPath.replace('_v3', '_v2'));
  
  // Count existing claims before
  const claimsBefore = kb.claims.length;
  const existingClaimIds = new Set(kb.claims.map(c => c.claim_id));
  const epassportClaimsBefore = kb.claims.filter(c => 
    c.claim_id.includes('epassport') || c.claim_id.includes('svc_epassport') || c.claim_id.includes('www_epassport')
  ).length;
  
  console.log(`  Claims in KB before Run #2: ${claimsBefore}`);
  console.log(`  epassport claims: ${epassportClaimsBefore}\n`);

  // Run stats
  const runStats = {
    runId: `run_${getDateString()}_pilot2_${Date.now()}`,
  ...
  for (const page of SCRAPED_PAGES) {
    try {
      ...
      const newClaimIds = claims.map(c => c.claim_id);
      const duplicates = newClaimIds.filter(id => existingClaimIds.has(id));
      runStats.duplicateClaimIds.push(...duplicates);
      
      // Add claims to KB (will skip duplicates based on claim_id)
      const claimsBeforeAdd = kb.claims.length;
      const addedClaims = addClaimsToKB(kb, claims);
      const claimsAfterAdd = kb.claims.length;
      
      const newClaims = claimsAfterAdd - claimsBeforeAdd;
      runStats.claimsWritten += newClaims;
      runStats.claimsSkipped += (addedClaims - newClaims);
      
      // Update existingClaimIds for next iteration
      for (const c of claims) {
        existingClaimIds.add(c.claim_id);
      }
      
      console.log(`     ✓ Extracted: ${structuredData.steps?.length || 0} steps, ${structuredData.faqPairs?.length || 0} FAQs`);
      console.log(`     ✓ Claims: ${newClaims} new, ${duplicates.length} duplicate (skipped)`);
  ...
  const epassportClaimsAfter = kb.claims.filter(c => 
    c.claim_id.includes('epassport') || c.claim_id.includes('svc_epassport') || c.claim_id.includes('www_epassport')
  ).length;

  // Generate run report
  const dateStr = getDateString();
  const runDir = path.join(PATHS.runsDir, `run_2_${dateStr}`);
  ensureDir(runDir);
  ...
  const report = {
    ...
    config: {
      ...
      note: 'KB NOT deleted between runs - idempotency test'
    },
    summary: {
      ...
      duplicate_claim_ids_found: runStats.duplicateClaimIds.length,
      errors: runStats.errors.length,
    },
    kb_state: {
      total_claims_before: claimsBefore,
      total_claims_after: claimsAfter,
      epassport_claims_before: epassportClaimsBefore,
      epassport_claims_after: epassportClaimsAfter,
      net_new_claims: claimsAfter - claimsBefore,
    },
    determinism_check: {
      all_duplicate_ids_matched: runStats.duplicateClaimIds.length === runStats.claimsSkipped,
      new_claims_added: runStats.claimsWritten,
      expected_new_claims: 0,
      idempotency_pass: runStats.claimsWritten === 0,
    },
  ...
  // Print summary
  console.log('\n' + '═'.repeat(70));
  console.log('  PILOT RUN #2 SUMMARY (IDEMPOTENCY TEST)');
  console.log('═'.repeat(70));
  ...
  console.log(`  Claims written (NEW): ${runStats.claimsWritten}`);
  console.log(`  Claims skipped (DUPLICATE): ${runStats.claimsSkipped}`);
  console.log(`  Errors: ${runStats.errors.length}`);
  console.log('─'.repeat(70));
  console.log(`  KB Claims Before: ${claimsBefore}`);
  console.log(`  KB Claims After: ${claimsAfter}`);
  console.log(`  Net New Claims: ${claimsAfter - claimsBefore}`);
  console.log('─'.repeat(70));
  console.log(`  ✅ IDEMPOTENCY TEST: ${runStats.claimsWritten === 0 ? 'PASSED' : 'FAILED'}`);
  console.log('═'.repeat(70) + '\n');
...
// Run
runPilot2().then(report => {
  console.log('✅ Pilot Run #2 complete');
  console.log(JSON.stringify(report, null, 2));
}).catch(err => {
  console.error('❌ Pilot Run #2 failed:', err);
  process.exit(1);
});
